  ...
  return (
    <div className="space-y-4">
      <div className="flex flex-wrap items-end justify-between gap-4">
        ...
        <label className="text-sm font-medium">
          ...
          <input
            ...
            className="ml-2 lw-input"
          />
        </label>
      </div>

      {saveError && (
        <div className="rounded border border-lw-bad bg-lw-bad/10 px-4 py-3 text-lw-bad">
      ...
      {lanes.every((l) => l.currentFrames === null) && (
        <div className="rounded-lg border border-lw-line2 bg-lw-warn/10 px-4 py-3 text-sm text-lw-text">
      ...
      <div className="lw-card overflow-hidden">
        ...
        <table className="lw-zebra w-full min-w-[34rem] text-left">
          ...
          <tbody>
            {lanes.map((lane, index) => {
              ...
              return (
                <tr key={lane.laneId} className="border-t border-lw-line">
                  <td className="px-4 py-2 font-semibold whitespace-nowrap">
                    Bahn {lane.laneNumber}
                    {lane.status !== 'active' && (
                      <span className="ml-2 rounded bg-lw-card2 px-1.5 py-0.5 text-xs font-medium text-lw-text2">
                        {lane.status === 'renovation' ? 'Renovierung' : 'außer Betrieb'}
                      </span>
                    )}
                  </td>
                  <td className="tabular px-4 py-2 text-right text-lw-text2">
                    {lane.lastRawValue === null ? '—' : formatFrames(lane.lastRawValue)}
                    <span className="ml-2 hidden text-xs text-lw-text3 sm:inline">
                  ...
                  <td className="px-4 py-2">
                    <input
                      ...
                      className={`tabular w-36 rounded border px-3 py-2 text-right text-lg ${
                        error ? 'border-lw-bad bg-lw-bad/10' : warning ? 'border-lw-warn bg-lw-warn/10' : 'border-lw-line'
                      }`}
                      placeholder="—"
                    />
                  </td>
                  <td className="px-4 py-2 text-sm">
                    {check?.delta !== null && check?.delta !== undefined && (
                      <span className="tabular font-semibold text-lw-text">
                        {check.delta >= 0 ? '+' : ''}
                    ...
                    {error && <div className="text-lw-bad">■ {error.message}</div>}
                    {!error && warning && <div className="text-lw-warn">▲ {warning.message}</div>}
                    {!error && !warning && info && <div className="text-lw-text3">– {info.message}</div>}
                  </td>
                </tr>
              );
            })}
      ...
      <div className="sticky bottom-0 flex flex-wrap items-center gap-4 border-t border-lw-line bg-lw-card2/95 py-3">
        ...
        <button
          ...
          className="ml-auto lw-btn-primary px-5 py-3 font-semibold disabled:opacity-40"
        >
...
function ResetDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/70 p-4">
      <div className="w-full max-w-xl lw-card p-5 shadow-xl">
        <h2 className="text-lg font-semibold text-lw-bad">■ Zählerstand niedriger als bisher</h2>
        ...
        <ul className="mt-3 space-y-2 text-sm">
          {candidates.map((c) => {
            ...
            return (
              <li key={c.laneId} className="rounded bg-lw-card2 px-3 py-2">
                ...
                <label className="mt-2 flex flex-wrap items-center gap-2">
                  ...
                  <input
                    ...
                    className="tabular w-24 rounded border border-lw-line px-2 py-1 text-right"
                  />
                </label>
                {gained >= 0 ? (
                  <div className="tabular mt-1 text-xs text-lw-text2">
                    Seit dem Wechsel gelaufen: {formatFrames(gained)} — neuer Gesamtstand{' '}
                    {formatFrames((lane?.currentFrames ?? 0) + gained)}
                  </div>
                ) : (
                  <div className="mt-1 text-xs text-lw-bad">
        ...
        <p className="mt-4 text-sm text-lw-text2">
          Der neue Frame-Stand ist niedriger als der bisherige Stand. Wurde der Zähler zurückgesetzt oder
          ausgetauscht?
        </p>
        <p className="mt-2 rounded border border-lw-ok bg-lw-ok/10 px-3 py-2 text-sm text-lw-ok">
        ...
        <div className="mt-5 flex flex-wrap justify-end gap-2">
          <button onClick={onCancel} className="rounded px-4 py-2 text-sm font-medium hover:bg-lw-card2">
